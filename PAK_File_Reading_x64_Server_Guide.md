...
## 9. Triển Khai Thư Mục Server x64
...
ExpandPackage.dll
│
├── package.ini                    ← cấu hình PAK
├── pak/                           ← thư mục chứa file .pak
│   ├── maps.pak
│   └── ...
│
├── settings/
│   ├── MapList.ini                ← ánh xạ ID bản đồ → đường dẫn
│   └── ...
│
└── maps/                          ← file bản đồ rời (fallback)
    └── ...
```
...
| Mode | Thứ tự | Ghi chú |
|------|--------|---------|
| 0 (mặc định) | Đĩa → PAK | Nếu file tồn tại trên đĩa, dùng luôn, không tìm PAK |
| 1 | PAK → Đĩa | Ưu tiên PAK, chỉ đọc đĩa nếu không tìm thấy trong PAK |

Nếu `package.ini` không tồn tại hoặc không có file PAK nào → server **vẫn hoạt động bình thường** bằng cách đọc file rời từ `\maps\`.
...
- [ ] `Engine\Src\KPakFile.h` — gỡ 2 guard `#ifndef _SERVER` (giữ guard SPR/JPEG)
- [ ] `Engine\Src\KPakFile.cpp` — gỡ 9 guard `#ifndef _SERVER` (giữ guard SPR/JPEG)
- [ ] `Core\Src\KSubWorld.cpp:641` — bỏ comment `g_PakList.Open("\\package.ini")`
- [ ] `Core\Src\CoreUseNameDef.h:139` — sửa `WorldSet.ini` → `MapList.ini`

### Build

- [ ] Clean toàn bộ solution (Build → Clean Solution)
- [ ] Xóa `Build/Lib/` thủ công để đảm bảo không còn lib cũ
- [ ] Build config **"Server Release x64"** trong `JXAll.sln`
- [ ] Kiểm tra **tất cả** DLL/EXE x64 có cùng timestamp
- [ ] Nếu build cả Win32: rebuild **"Server Release"** và **"Client Release"** để ABI nhất quán

### Triển khai

- [ ] Copy tất cả file `*64.exe` và `*64.dll` vào thư mục server
- [ ] Tạo `package.ini` với đường dẫn đúng đến thư mục PAK
- [ ] Tạo thư mục `pak/` và copy các file `.pak` vào
- [ ] Đảm bảo `settings/MapList.ini` tồn tại (không phải `WorldSet.ini`)
- [ ] Đảm bảo `rainbow64.dll` tồn tại (thiếu → crash trước khi load PAK)
- [ ] Không cần cài VC++ Redistributable (đã dùng /MT)

### Kiểm tra

- [ ] Khởi động server → kiểm tra log có `"PakList Open : ... Ok"` cho mỗi file PAK
- [ ] Kiểm tra log `"PakList: N PAK files loaded"` — số N phải khớp số entry trong `package.ini`
- [ ] Load thử một bản đồ → kiểm tra region và object data load thành công
- [ ] Nếu log hiện `"... Fail"` → kiểm tra đường dẫn, quyền đọc, và signature file PAK ("PACK" = `0x4B434150`)
